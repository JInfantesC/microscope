...
Router.route("/:postsLimit?", {
    //We define a new route when we are at / that will load template postsList
    /*We'll then add a postsLimit parameter to the route's path.
    Adding a ? after the parameter name means that it's optional.
    So our route will not only match http://localhost:3000/50, but also plain
    ...
    */
    ...
    /*
    And since we're setting the data context at the route level, we can now
    safely get rid of the posts template helper inside the posts_list.js file
    and just delete the contents of that file.
    */
    data: function() {
        var limit = parseInt(this.params.postsLimit) || 5;
        return {
